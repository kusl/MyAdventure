...
public class OverflowRegressionTests
{
    ...
    /// <summary>
    /// Chain five back-to-back max-cap buys against effectively infinite
    /// cash. Owned must stay strictly positive and monotonically increase
    /// after every batch. This is the closure of the production scenario:
    /// the original bug surfaced because each successive bulk-buy
    /// compounded the corruption.
    /// </summary>
    [Fact]
    public async Task BuyMultiple_RepeatedAtCap_OwnedStaysPositiveAndIncreases()
    {
        await _engine.LoadAsync();
        SetCash(new BigDouble(1.0, 10_000_000));
        ...
        for (var i = 0; i < 5; i++)
        {
            ...
            biz.Owned.ShouldBeGreaterThan(last);
